...
#include "AI/SlAiEnemyBTTaskAttackFollow.h"
...
EBTNodeResult::Type USlAiEnemyBTTaskAttackFollow::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	//如果初始化敌人参数不成功,直接返回失败
	...
	else
	{
		...
		OwnerComp.GetBlackboardComponent()->SetValueAsVector(Destination.SelectedKeyName, SECharactor->GetActorLocation());
	}
	//返回成功
